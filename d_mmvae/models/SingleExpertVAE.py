...
class Model(nn.Module):
    ...
    def __init__(self, experts: nn.ModuleDict, solo_vae: M.VAE):
        super().__init__()
    ...
    def forward(self, train_input: torch.Tensor):
        ...
        shared_output, mu, var, shared_encoder_outputs, shared_decoder_outputs = self.solo_vae(shared_input)
...
def configure_model(num_experts = 1) -> Model:
        return Model(
            nn.ModuleDict({ 
                    'expert': M.Expert(
                        nn.Sequential(

...
class PreTrained_Model():
    ...
    def load_pretrained_model(self):
        ...
        #Loading snapshot
        expert = self.load_snapshot(SNAPSHOT_EXPERT_PATH)
        shared_encoder = self.load_snapshot(SNAPSHOT_SHARED_PATH+'_encoder')
        shared_decoder = self.load_snapshot(SNAPSHOT_SHARED_PATH+'_decoder')
        shared_var = self.load_snapshot(SNAPSHOT_SHARED_PATH+'_var')
        shared_mean = self.load_snapshot(SNAPSHOT_SHARED_PATH+'_mean')
        
        self.model.model.expert.load_state_dict(expert)
        self.model.model.shared_vae.encoder.load_state_dict(shared_encoder)
        self.model.model.shared_vae.decoder.load_state_dict(shared_decoder)
        self.model.model.shared_vae.var.load_state_dict(shared_var)
        self.model.model.shared_vae.mean.load_state_dict(shared_mean)
        

    def load_snapshot(self, path) -> tuple[nn.Module]:
...
def main(device):
    ...
    for param in ft_trainer.model.model.expert.parameters():
        param.requires_grad = False
    for param in ft_trainer.model.model.shared_vae.encoder.parameters():
        param.requires_grad = False
    for param in ft_trainer.model.model.shared_vae.decoder.parameters():
        param.requires_grad = False
